...
class TestView(TestCase) :
    ...
    def test_create_post(self) :
        ...
        self.assertEqual(last_post.author.username, 'desmos')
    ...
    def test_update_post(self) :
        ...
        # login but not writer
        self.assertNotEqual(self.post_003.author, self.user_mathway)
        self.client.login(
            username = self.user_mathway.username,
            password = 'pass'
        )
        response = self.client.get(update_post_url)
        ...
        # if writer access
        self.client.login(
            username = self.user_desmos.username,
            password = 'pass'
        )
        response = self.client.get(update_post_url)
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_comment(self) :
        ...
        self.assertIn(self.post_001.title, soup.title.text)
        comment_area = soup.find('div', id = "comment-area")
        self.assertNotIn("Mathway's comment", comment_area.text)
